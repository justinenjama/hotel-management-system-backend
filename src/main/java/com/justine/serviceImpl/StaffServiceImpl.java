...
public class StaffServiceImpl implements StaffService {
    ...
    private boolean isAdmin(String email) {
        ...
        return staff.isPresent() && staff.get().getRole() == StaffRole.ADMIN;
    }
    ...
    // ============ ADD STAFF ============
    @Override
    public ResponseEntity<StaffResponseDTO> addStaff(StaffRequestDTO dto, String currentUserEmail) {
        ...
        try {
            ...
            auditLogService.logStaff(actorId, "ADD_STAFF_SUCCESS", staff.getId(),
                    Map.of("user", currentUserEmail, "email", dto.getEmail()));
    ...
    // ============ UPDATE STAFF ============
    @Override
    public ResponseEntity<StaffResponseDTO> updateStaff(Long staffId, StaffRequestDTO dto, String currentUserEmail) {
        Long actorId = getActorId(currentUserEmail);
        try {
            Staff staff = staffRepository.findById(staffId)
                    .orElseThrow(() -> new NoSuchElementException("Staff not found"));

            if (!isAdmin(currentUserEmail) && !staff.getEmail().equals(currentUserEmail)) {
            ...
            if (dto.getRole() != null && isAdmin(currentUserEmail)) {
            ...
            auditLogService.logStaff(actorId, "UPDATE_STAFF_SUCCESS", staffId,
                    Map.of("user", currentUserEmail, "role", staff.getRole().toString()));
    ...
    // ============ DELETE STAFF ============
    @Override
    public ResponseEntity<Void> deleteStaff(Long staffId, String currentUserEmail) {
        Long actorId = getActorId(currentUserEmail);
        try {
            if (!isAdmin(currentUserEmail)) {
    ...
    @Override
    public ResponseEntity<StaffResponseDTO> assignStaffToHotel(Long staffId, Long hotelId, String currentUserEmail) {
        Long actorId = getActorId(currentUserEmail);
        try {
            if (!isAdmin(currentUserEmail)) {
            ...
            auditLogService.logStaff(actorId, "ASSIGN_STAFF_SUCCESS", staffId,
                    Map.of("hotelId", hotelId, "user", currentUserEmail));

            return ResponseEntity.ok(toStaffResponse(staff));

        } catch (Exception e) {
            auditLogService.logStaff(actorId, "ASSIGN_STAFF_FAILED", staffId,
                    Map.of("hotelId", hotelId, "user", currentUserEmail, "error", e.getMessage()));
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    ...
    // ============ GET STAFF BY ID ============
    @Override
    public ResponseEntity<StaffResponseDTO> getStaffById(Long staffId, String currentUserEmail) {
        Long actorId = getActorId(currentUserEmail);
        try {
            Staff staff = staffRepository.findById(staffId)
                    .orElseThrow(() -> new NoSuchElementException("Staff not found"));
    ...
    // ============ GET ALL STAFF ============
    @Override
    public ResponseEntity<List<StaffResponseDTO>> getAllStaff(String currentUserEmail) {
        Long actorId = getActorId(currentUserEmail);
        try {
            if (!isAdmin(currentUserEmail)) {
            ...
            auditLogService.logStaff(actorId, "GET_ALL_STAFF_SUCCESS", null,
                    Map.of("count", staffList.size(), "user", currentUserEmail));
